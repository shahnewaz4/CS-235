...
void Novel::display(){
  ...
  std::string film = "It has";
	if(!isDigital())
	{
		digital = "It is not";
	}
  if(!hasFilmAdaptation()){
    film = "It does not have";

  }
	std::cout << getTitle() << " is written by " << getAuthor() << ". Genre: " << getGenre() << ". " << film << " a film adaptation. Page Count: " << getPageCount() << ". " << digital << " available digitally.\n";
}
